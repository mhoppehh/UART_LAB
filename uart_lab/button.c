...
uint8_t button_getButton() {
    ...
    //uint32_t temp = (GPIO_PORTE_DATA_R & 0b00000100);l

    if((GPIO_PORTE_DATA_R & 0b00100000) != 0x20) //0x means hex; We used hex for convenience and because the TA hates on us because decimal would have been easier to read!!!!!!!!!!!!! ;P
        return 6;
    if((GPIO_PORTE_DATA_R & 0b00010000) != 0x10)
        return 5;
    if((GPIO_PORTE_DATA_R & 0b00001000) != 0x08)
        return 4;
    if((GPIO_PORTE_DATA_R & 0b00000100) != 0x04)
        return 3;
    if((GPIO_PORTE_DATA_R & 0b00000010) != 0x02)
        return 2;
    if((GPIO_PORTE_DATA_R & 0b00000001) != 0x01)
        return 1;
